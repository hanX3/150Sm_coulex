//
void get_par_single(ofstream &fo, int run, int cid, int sid, int ch, bool flag_3a_2a)
{
  TString str_tmp, str_out;
  TH1D *h1;

  str_tmp = Form("../rootfile/si/data%04d_hist.root", run);
...
{
  ofstream fo(TString::Format("cali_%04d.txt",run).Data(), std::ios::app);
  fo << " cid   sid    ch                p0                p1                p2               chi" << endl;

  int cid = 1;
  for(int sid=2;sid<=8;sid++){
    for(int ch=0;ch<16;ch++){
      get_par_single(fo, run, cid, sid, ch, 1);
    }
  }
  for(int sid=9;sid<=12;sid++){
    for(int ch=0;ch<16;ch++){
      get_par_single(fo, run, cid, sid, ch, 0);
    }
  }
