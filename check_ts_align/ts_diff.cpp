...
void ts_diff(int run, int win)
{
  ...
  //
  TFile *fo = new TFile(TString::Format("../rootfile/data%04d_align_%dns_ts_diff.root",run,win).Data(), "recreate");
  
  TFile *fi = TFile::Open(TString::Format("../rootfile/data%04d_align_%dns_%s.root", run, TIMEWINDOW, TF).Data());
  if(fi->IsZombie()){
