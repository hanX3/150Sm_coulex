...
//
void draw_sample_win_jump(int run, string str="")
{
  // gROOT->SetBatch(1);
  ...
  vector<pair<int, int>> v_win_jump = {
    {50, 300},
    {100, 300},
    {150, 300},
    {200, 300},
  ...
  int nn = v_win_jump.size();
  cout << "nn " << nn << endl;

  //
  TFile *fi[12];
  for(size_t i=0;i<nn;++i){
    fi[i] = TFile::Open(TString::Format("../rootfile/data%04d_doppler_%dns_jump_%dns_%s_hist.root",run,v_win_jump[i].first,v_win_jump[i].second, str.c_str()));
  ...
  for(int i=0;i<nn;i++){
    ...
    leg1->AddEntry(h1[i], TString::Format("win%dns_jump%dns",v_win_jump[i].first,v_win_jump[i].second));
  }
  leg1->Draw("same");
  c1->SaveAs(TString::Format("./fig/%04d/%s_win_jump_e_raw.png", run, str.c_str()).Data());
  ...
  for(int i=0;i<nn;i++){
    ...
    leg2->AddEntry(h2[i], TString::Format("win%dns_jump%dns",v_win_jump[i].first,v_win_jump[i].second));
  }
  leg2->Draw("same");
  c2->SaveAs(TString::Format("./fig/%04d/%s_win_jump_e_dc_p_1000fs.png", run, str.c_str()).Data());
  ...
  for(int i=0;i<nn;i++){
    ...
    leg3->AddEntry(h3[i], TString::Format("win%dns_jump%dns",v_win_jump[i].first,v_win_jump[i].second));
  }
  leg3->Draw("same");
  c3->SaveAs(TString::Format("./fig/%04d/%s_win_jump_e_dc_r_1000fs.png", run, str.c_str()).Data());
  ...
  for(int i=0;i<nn;++i){
    ...
    leg4->Draw("same");
    c4->SaveAs(TString::Format("./fig/%04d/%s_win%dns_jump%dns_e_raw_e_dc_p_1000fs.png", run, str.c_str(), v_win_jump[i].first, v_win_jump[i].second).Data());
  }
  ...
  for(int i=0;i<nn;++i){
    ...
    leg5->Draw("same");
    c5->SaveAs(TString::Format("./fig/%04d/%s_win%dns_jump%dns_e_raw_e_dc_r_1000fs.png", run, str.c_str(), v_win_jump[i].first, v_win_jump[i].second).Data());
  }
  ...
  for(int i=0;i<nn;++i){
    ...
    c9->cd();
    leg9->Draw("same");
    c9->SaveAs(TString::Format("./fig/%04d/%s_win%dns_jump%dns_e_raw_diff_spider_ring.png", run, str.c_str(), v_win_jump[i].first, v_win_jump[i].second).Data());

    c10->cd();
    leg10->Draw("same");
    c10->SaveAs(TString::Format("./fig/%04d/%s_win%dns_jump%dns_e_dc_p_diff_spider_ring.png", run, str.c_str(), v_win_jump[i].first, v_win_jump[i].second).Data());

    c11->cd();
    leg11->Draw("same");
    c11->SaveAs(TString::Format("./fig/%04d/%s_win%dns_jump%dns_e_dc_r_diff_spider_ring.png", run, str.c_str(), v_win_jump[i].first, v_win_jump[i].second).Data());
  }
  ...
  for(int i=0;i<nn;++i){
    ...
    c12->cd();
    leg12->Draw("same");
    c12->SaveAs(TString::Format("./fig/%04d/%s_win%dns_jump%dns_e_raw_diff_spider_sector.png", run, str.c_str(), v_win_jump[i].first, v_win_jump[i].second).Data());

    c13->cd();
    leg13->Draw("same");
    c13->SaveAs(TString::Format("./fig/%04d/%s_win%dns_jump%dns_e_dc_p_1000fs_diff_spider_sector.png", run, str.c_str(), v_win_jump[i].first, v_win_jump[i].second).Data());

    c14->cd();
    leg14->Draw("same");
    c14->SaveAs(TString::Format("./fig/%04d/%s_win%dns_jump%dns_e_dc_r_1000fs_diff_spider_sector.png", run, str.c_str(), v_win_jump[i].first, v_win_jump[i].second).Data());
  }
