...
//
void d2hist(int run, int win)
{
  TRandom3 *rndm = new TRandom3((Long64_t)time(0)); 

  TFile *fi = TFile::Open(TString::Format("./try/%03dns/rootfile/data%04d_doppler_%dns_jump_300ns.root",win,run,win));
  TFile *fo = new TFile(TString::Format("./try/%03dns/rootfile/data%04d_doppler_%dns_jump_300ns_hist.root",win,run,win), "recreate");
