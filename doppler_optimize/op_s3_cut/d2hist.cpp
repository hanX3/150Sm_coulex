  ...
  for(auto it=v_str_name.begin();it!=v_str_name.end();++it){
    ...
    for(int i=1;i<=24;i++){
      ...
      for(auto &str_e : v_str_e){
        // ge all, s3 ring
        TH1D h1(Form("%s_%s_ge_all_s3_ring%d",(*it).c_str(),str_e.c_str(),i), "", 4096, 0, 4096);
        tr->Project(h1.GetName(), str_e.c_str(), Form("!flag_spider_p && flag_s3_p && rp==%d",i));
        h1.Write();

        // ge ring, s3 ring
        TH1D h3(Form("%s_%s_ge_ring3_s3_ring%d",(*it).c_str(),str_e.c_str(),i), "", 4096, 0, 4096);
        tr->Project(h3.GetName(), str_e.c_str(), Form("!flag_spider_p && flag_s3_p && rp==%d && rg==3",i));
        h3.Write();

        // ge ring, s3 ring
        TH1D h4(Form("%s_%s_ge_ring4_s3_ring%d",(*it).c_str(),str_e.c_str(),i), "", 4096, 0, 4096);
        tr->Project(h4.GetName(), str_e.c_str(), Form("!flag_spider_p && flag_s3_p && rp==%d && rg==4",i));
        h4.Write();

        // ge ring, s3 ring
        TH1D h5(Form("%s_%s_ge_ring5_s3_ring%d",(*it).c_str(),str_e.c_str(),i), "", 4096, 0, 4096);
        tr->Project(h5.GetName(), str_e.c_str(), Form("!flag_spider_p && flag_s3_p && rp==%d && rg==5",i));
        h5.Write();

        // ge single, s3 ring
        TH1D h32(Form("%s_%s_ge_ring3_sector2_s3_ring%d",(*it).c_str(),str_e.c_str(),i), "", 4096, 0, 4096);
        tr->Project(h32.GetName(), str_e.c_str(), Form("!flag_spider_p && flag_s3_p && rp==%d && rg==3 && sg==2",i));
        h32.Write();

        // ge single, s3 ring
        TH1D h41(Form("%s_%s_ge_ring4_sector1_s3_ring%d",(*it).c_str(),str_e.c_str(),i), "", 4096, 0, 4096);
        tr->Project(h41.GetName(), str_e.c_str(), Form("!flag_spider_p && flag_s3_p && rp==%d && rg==4 && sg==1",i));
        h41.Write();

        // ge single, s3 ring
        TH1D h52(Form("%s_%s_ge_ring5_sector2_s3_ring%d",(*it).c_str(),str_e.c_str(),i), "", 4096, 0, 4096);
