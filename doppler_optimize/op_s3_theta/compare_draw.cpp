void compare_draw()
{
  ...
  for(int i=0;i<n_files;++i){
    ...
    if(i==0){
      v_h_ga_s3a[i]->SetTitle("ge all, different s3 theta offset");
  ...
  for(int i=0;i<n_files;++i){
    ...
    v_h_grs32_s3a[i]->SetTitle("ge ring3 sector2, different s3 theta offset");
  ...
  // ge ring3 sector2, s3 different ring, s3 theta different offset
  TCanvas *cc_grs32_s3r[n_files];
  TLegend *leg_grs32_s3r[n_files];
  for(int r=0;r<8;r++){
    cc_grs32_s3r[r] = new TCanvas(Form("cc_grs32_s3r%d",(r+1)*3),"",900,500);
    cc_grs32_s3r[r]->cd();
    cc_grs32_s3r[r]->SetLogy();

    leg_grs32_s3r[r] = new TLegend(0.75, 0.38, 0.88, 0.92);

    for(int i=0;i<n_files;i++){
      leg_grs32_s3r[r]->AddEntry(v_h_grs32_s3r[8*i+r], Form("%s",v_filename[i].substr(4,6).c_str()));
      v_h_grs32_s3r[8*i+r]->SetLineColor(colors[i]);
      v_h_grs32_s3r[8*i+r]->SetLineWidth(2);
      v_h_grs32_s3r[8*i+r]->GetXaxis()->SetRangeUser(300,500);
      if(i==0){
        v_h_grs32_s3r[8*i+r]->SetTitle(Form("ge ring3 sector2, s3 ring%d, s3 theta different offset",(r+1)*3));
        v_h_grs32_s3r[8*i+r]->GetXaxis()->SetTitle("energy [keV]");
        v_h_grs32_s3r[8*i+r]->GetYaxis()->SetTitle("counts / keV");
        v_h_grs32_s3r[8*i+r]->GetYaxis()->SetRangeUser(1., 10000);
        v_h_grs32_s3r[8*i+r]->Draw("hist");
      }else v_h_grs32_s3r[8*i+r]->Draw("same hist");
    }

    leg_grs32_s3r[r]->Draw("same");

    l_334->Draw("same");
    l_406->Draw("same");
    l_439->Draw("same");

    cc_grs32_s3r[r]->SaveAs(Form("fig/%s.png",cc_grs32_s3r[r]->GetName()));
  }
  ...
  for(int i=0;i<n_files;++i){
    ...
    if(i==0){
      ...
      v_h_grs41_s3a[i]->GetYaxis()->SetRangeUser(1., 10000);
  ...
  // ge ring4 sector1, s3 different ring, s3 theta different offset
  TCanvas *cc_grs41_s3r[n_files];
  TLegend *leg_grs41_s3r[n_files];
  for(int r=0;r<8;r++){
    cc_grs41_s3r[r] = new TCanvas(Form("cc_grs41_s3r%d",(r+1)*3),"",900,500);
    cc_grs41_s3r[r]->cd();
    cc_grs41_s3r[r]->SetLogy();

    leg_grs41_s3r[r] = new TLegend(0.75, 0.38, 0.88, 0.92);

    for(int i=0;i<n_files;i++){
      leg_grs41_s3r[r]->AddEntry(v_h_grs41_s3r[8*i+r], Form("%s",v_filename[i].substr(4,6).c_str()));
      v_h_grs41_s3r[8*i+r]->SetLineColor(colors[i]);
      v_h_grs41_s3r[8*i+r]->SetLineWidth(2);
      v_h_grs41_s3r[8*i+r]->GetXaxis()->SetRangeUser(300,500);
      if(i==0){
        v_h_grs41_s3r[8*i+r]->SetTitle(Form("ge ring4 sector1, s3 ring%d, s3 theta different offset",(r+1)*3));
        v_h_grs41_s3r[8*i+r]->GetXaxis()->SetTitle("energy [keV]");
        v_h_grs41_s3r[8*i+r]->GetYaxis()->SetTitle("counts / keV");
        v_h_grs41_s3r[8*i+r]->GetYaxis()->SetRangeUser(1., 10000);
        v_h_grs41_s3r[8*i+r]->Draw("hist");
      }else v_h_grs41_s3r[8*i+r]->Draw("same hist");
    }

    leg_grs41_s3r[r]->Draw("same");

    l_334->Draw("same");
    l_406->Draw("same");
    l_439->Draw("same");

    cc_grs41_s3r[r]->SaveAs(Form("fig/%s.png",cc_grs41_s3r[r]->GetName()));
  }
  ...
  for(int i=0;i<n_files;++i){
    ...
    if(i==0){
      ...
      v_h_grs42_s3a[i]->GetYaxis()->SetRangeUser(1., 10000);
  ...
  // ge ring4 sector2, s3 different ring, s3 theta different offset
  TCanvas *cc_grs42_s3r[n_files];
  TLegend *leg_grs42_s3r[n_files];
  for(int r=0;r<8;r++){
    cc_grs42_s3r[r] = new TCanvas(Form("cc_grs42_s3r%d",(r+1)*3),"",900,500);
    cc_grs42_s3r[r]->cd();
    cc_grs42_s3r[r]->SetLogy();

    leg_grs42_s3r[r] = new TLegend(0.75, 0.38, 0.88, 0.92);

    for(int i=0;i<n_files;i++){
      leg_grs42_s3r[r]->AddEntry(v_h_grs42_s3r[8*i+r], Form("%s",v_filename[i].substr(4,6).c_str()));
      v_h_grs42_s3r[8*i+r]->SetLineColor(colors[i]);
      v_h_grs42_s3r[8*i+r]->SetLineWidth(2);
      v_h_grs42_s3r[8*i+r]->GetXaxis()->SetRangeUser(300,500);
      if(i==0){
        v_h_grs42_s3r[8*i+r]->SetTitle(Form("ge ring4 sector2, s3 ring%d, s3 theta different offset",(r+1)*3));
        v_h_grs42_s3r[8*i+r]->GetXaxis()->SetTitle("energy [keV]");
        v_h_grs42_s3r[8*i+r]->GetYaxis()->SetTitle("counts / keV");
        v_h_grs42_s3r[8*i+r]->GetYaxis()->SetRangeUser(1., 10000);
        v_h_grs42_s3r[8*i+r]->Draw("hist");
      }else v_h_grs42_s3r[8*i+r]->Draw("same hist");
    }

    leg_grs42_s3r[r]->Draw("same");

    l_334->Draw("same");
    l_406->Draw("same");
    l_439->Draw("same");

    cc_grs42_s3r[r]->SaveAs(Form("fig/%s.png",cc_grs42_s3r[r]->GetName()));
  }
  ...
  for(int i=0;i<n_files;++i){
    ...
    if(i==0){
      ...
      v_h_grs43_s3a[i]->GetYaxis()->SetRangeUser(1., 10000);
  ...
  // ge ring4 sector3, s3 different ring, s3 theta different offset
  TCanvas *cc_grs43_s3r[n_files];
  TLegend *leg_grs43_s3r[n_files];
  for(int r=0;r<8;r++){
    cc_grs43_s3r[r] = new TCanvas(Form("cc_grs43_s3r%d",(r+1)*3),"",900,500);
    cc_grs43_s3r[r]->cd();
    cc_grs43_s3r[r]->SetLogy();

    leg_grs43_s3r[r] = new TLegend(0.75, 0.38, 0.88, 0.92);

    for(int i=0;i<n_files;i++){
      leg_grs43_s3r[r]->AddEntry(v_h_grs43_s3r[8*i+r], Form("%s",v_filename[i].substr(4,6).c_str()));
      v_h_grs43_s3r[8*i+r]->SetLineColor(colors[i]);
      v_h_grs43_s3r[8*i+r]->SetLineWidth(2);
      v_h_grs43_s3r[8*i+r]->GetXaxis()->SetRangeUser(300,500);
      if(i==0){
        v_h_grs43_s3r[8*i+r]->SetTitle(Form("ge ring4 sector3, s3 ring%d, s3 theta different offset",(r+1)*3));
        v_h_grs43_s3r[8*i+r]->GetXaxis()->SetTitle("energy [keV]");
        v_h_grs43_s3r[8*i+r]->GetYaxis()->SetTitle("counts / keV");
        v_h_grs43_s3r[8*i+r]->GetYaxis()->SetRangeUser(1., 10000);
        v_h_grs43_s3r[8*i+r]->Draw("hist");
      }else v_h_grs43_s3r[8*i+r]->Draw("same hist");
    }

    leg_grs43_s3r[r]->Draw("same");

    l_334->Draw("same");
    l_406->Draw("same");
    l_439->Draw("same");

    cc_grs43_s3r[r]->SaveAs(Form("fig/%s.png",cc_grs43_s3r[r]->GetName()));
  }
  ...
  for(int i=0;i<n_files;++i){
    ...
    if(i==0){
      ...
      v_h_grs52_s3a[i]->GetYaxis()->SetRangeUser(1., 10000);
  ...
  // ge ring5 sector2, s3 different ring, s3 theta different offset
  TCanvas *cc_grs52_s3r[n_files];
  TLegend *leg_grs52_s3r[n_files];
  for(int r=0;r<8;r++){
    cc_grs52_s3r[r] = new TCanvas(Form("cc_grs52_s3r%d",(r+1)*3),"",900,500);
    cc_grs52_s3r[r]->cd();
    cc_grs52_s3r[r]->SetLogy();

    leg_grs52_s3r[r] = new TLegend(0.75, 0.38, 0.88, 0.92);

    for(int i=0;i<n_files;i++){
      leg_grs52_s3r[r]->AddEntry(v_h_grs52_s3r[8*i+r], Form("%s",v_filename[i].substr(4,6).c_str()));
      v_h_grs52_s3r[8*i+r]->SetLineColor(colors[i]);
      v_h_grs52_s3r[8*i+r]->SetLineWidth(2);
      v_h_grs52_s3r[8*i+r]->GetXaxis()->SetRangeUser(300,500);
      if(i==0){
        v_h_grs52_s3r[8*i+r]->SetTitle(Form("ge ring5 sector2, s3 ring%d, s3 theta different offset",(r+1)*3));
        v_h_grs52_s3r[8*i+r]->GetXaxis()->SetTitle("energy [keV]");
        v_h_grs52_s3r[8*i+r]->GetYaxis()->SetTitle("counts / keV");
        v_h_grs52_s3r[8*i+r]->GetYaxis()->SetRangeUser(1., 10000);
        v_h_grs52_s3r[8*i+r]->Draw("hist");
      }else v_h_grs52_s3r[8*i+r]->Draw("same hist");
    }

    leg_grs52_s3r[r]->Draw("same");

    l_334->Draw("same");
    l_406->Draw("same");
    l_439->Draw("same");

    cc_grs52_s3r[r]->SaveAs(Form("fig/%s.png",cc_grs52_s3r[r]->GetName()));
  }
  ...
  for(int i=0;i<n_files;++i){
    ...
    if(i==0){
      ...
      v_h_grs53_s3a[i]->GetYaxis()->SetRangeUser(1., 10000);
  ...
  // ge ring5 sector3, s3 different ring, s3 theta different offset
  TCanvas *cc_grs53_s3r[n_files];
  TLegend *leg_grs53_s3r[n_files];
  for(int r=0;r<8;r++){
    cc_grs53_s3r[r] = new TCanvas(Form("cc_grs53_s3r%d",(r+1)*3),"",900,500);
    cc_grs53_s3r[r]->cd();
    cc_grs53_s3r[r]->SetLogy();

    leg_grs53_s3r[r] = new TLegend(0.75, 0.38, 0.88, 0.92);

    for(int i=0;i<n_files;i++){
      leg_grs53_s3r[r]->AddEntry(v_h_grs53_s3r[8*i+r], Form("%s",v_filename[i].substr(4,6).c_str()));
      v_h_grs53_s3r[8*i+r]->SetLineColor(colors[i]);
      v_h_grs53_s3r[8*i+r]->SetLineWidth(2);
      v_h_grs53_s3r[8*i+r]->GetXaxis()->SetRangeUser(300,500);
      if(i==0){
        v_h_grs53_s3r[8*i+r]->SetTitle(Form("ge ring5 sector3, s3 ring%d, s3 theta different offset",(r+1)*3));
        v_h_grs53_s3r[8*i+r]->GetXaxis()->SetTitle("energy [keV]");
        v_h_grs53_s3r[8*i+r]->GetYaxis()->SetTitle("counts / keV");
        v_h_grs53_s3r[8*i+r]->GetYaxis()->SetRangeUser(1., 10000);
        v_h_grs53_s3r[8*i+r]->Draw("hist");
      }else v_h_grs53_s3r[8*i+r]->Draw("same hist");
    }

    leg_grs53_s3r[r]->Draw("same");

    l_334->Draw("same");
    l_406->Draw("same");
    l_439->Draw("same");

    cc_grs53_s3r[r]->SaveAs(Form("fig/%s.png",cc_grs53_s3r[r]->GetName()));
  }
}
