...
void tree2th(TString str, int run=0)
{
  ...
  double cut_ge_min = 20.; 
  double cut_ge_max = 4096.; 
  double cut_spider_min = 2000;
  double cut_spider_max = 100000;
  double cut_s3_min = 2000;
  double cut_s3_max = 100000;
  ...
  for(int i=2;i<=5;i++){
    for(int j=0;j<16;j++){
      hh_spider[(i-2)*16+j] = new TH2D(TString::Format("hh_spider_ge_sid%d_ch%02d",i,j).Data(), "", 200,-500,500,4096,0,4096);
      // hh_s3[(i-2)*16+j] = new TH2D(TString::Format("hh_s3_ge_sid%d_ch%02d",i,j).Data(), "", 200,-500,500,4096,0,4096);
    }
  }
    ...
    if(flag_spider){
      for(int j=0;j<n_ge;j++){  
        for(int jj=0;jj<v_spider_ts.size();jj++){
          hpg->Fill(v_spider_ts[jj]-ge_ts[j]);
