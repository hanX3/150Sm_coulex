...
void tree2th_bg_new(int run)
{
  ...
  TFile *fo = new TFile(TString::Format("../rootfile/data%04d_ge_tw_th2d_bg.root", run).Data(), "recreate");
  ...
  TH2D *hhpg = new TH2D("hhpg", "", 300,-1500,1500,1024,0,4096);
  ...
  for(Long64_t i=0;i<tr->GetEntries();i++){
    ...
    if(flag_ge && flag_spider){
      ...
      if(v_spider_ts[j]-v_ge_ts[0]>0){
        hpg->Fill(v_spider_ts[j]-v_ge_ts[0]-TIMEJUMP);
        hh_spider[(ge_sid[0]-2)*16+ge_ch[0]]->Fill(v_spider_ts[j]-v_ge_ts[0]-TIMEJUMP, ge_energy[0]);
        hhpg->Fill(v_spider_ts[j]-v_ge_ts[0]-TIMEJUMP, ge_energy[0]);
      }else{
        hpg->Fill(v_spider_ts[j]-v_ge_ts[0]+TIMEJUMP);
        hh_spider[(ge_sid[0]-2)*16+ge_ch[0]]->Fill(v_spider_ts[j]-v_ge_ts[0]+TIMEJUMP, ge_energy[0]);
        hhpg->Fill(v_spider_ts[j]-v_ge_ts[0]+TIMEJUMP, ge_energy[0]);
    ...
    if(flag_ge && flag_s3){
      ...
      if(v_s3_ts[j]-v_ge_ts[0]>0){
        hh_s3[(ge_sid[0]-2)*16+ge_ch[0]]->Fill(v_s3_ts[j]-v_ge_ts[0]-TIMEJUMP, ge_energy[0]);
      }else{
        hh_s3[(ge_sid[0]-2)*16+ge_ch[0]]->Fill(v_s3_ts[j]-v_ge_ts[0]+TIMEJUMP, ge_energy[0]);
      }
    }
