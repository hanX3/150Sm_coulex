...
void fit_ring(int run_first, int run_last, int ring_id)
{
  ...
  */
  ...
  for(int ii=0;ii<v_run.size()-1;ii++){
    TFile *fi_ref = TFile::Open(Form("../rootfile/si/data%04d_ring_hist.root", v_run[0]));
    ...
    */
    ...
    m_run_kx[v_run[ii+1]] = m_chi2_kx.begin()->second;
    ...
    cc->SaveAs(Form("./fig/ring%02d/run%04d_%04d.png", ring_id, v_run[0], v_run[ii+1]));
...
void fit_sector(int run_first, int run_last, int sector_id)
{
  ...
  */
  ...
  for(int ii=0;ii<v_run.size()-1;ii++){
    TFile *fi_ref = TFile::Open(Form("../rootfile/si/data%04d_sector_hist.root", v_run[0]));
    ...
    */
    ...
    cc->SaveAs(Form("./fig/sector%02d/run%04d_%04d.png", sector_id, v_run[0], v_run[ii+1]));
...
//
void fit_xy_new(int ring_id, int sector_id)
{
  gROOT->SetBatch(1);
