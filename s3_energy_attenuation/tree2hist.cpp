...
void tree2hist_sector(int run)
{
  ...
  TFile *fo = new TFile(TString::Format("../rootfile/si/data%04d_sector_hist.root", run).Data(), "recreate");
...
void tree2hist_ring(int run)
{
  ...
  TFile *fo = new TFile(TString::Format("../rootfile/si/data%04d_ring_hist.root", run).Data(), "recreate");
