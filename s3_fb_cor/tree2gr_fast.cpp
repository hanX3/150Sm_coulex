...
void tree2gr_sector(int run)
{
  ...
  for(int i=1;i<=32;i++){
    ...
    n = tr->Draw(draw_str.Data(), cut_str.Data(), "goff", tr->GetEntries()/8);
    cout << "n " << n << endl;
  ...
  TFile *fo = new TFile(TString::Format("../rootfile/si/data%04d_gr_fast.root", run).Data(), "recreate");
...
void tree2gr_ring(int run)
{
  ...
  TFile *fo = new TFile(TString::Format("../rootfile/si/data%04d_gr_fast.root", run).Data(), "update");
