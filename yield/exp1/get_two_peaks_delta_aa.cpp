...
std::map<int, std::vector<double>> m_2e_info;
...
// pars = 22
// two Gaussian + two Left-Skew + two Right-Skew + two Step + Background
Double_t g2peakexregion_new(Double_t *v, Double_t *par);
...
void get_two_peaks_delta_aa()
{
  ...
  for(auto &prefix:v_hist_prefix){
    ...
    cout << "m_2e_info " << m_2e_info.size() << endl;

    ofstream fo(Form("./%s_two_peaks_delta_aa.txt",prefix.c_str()));
    fo << "e   delta  s  beta  rho" << endl;

    for(auto &[key,val]:m_2e_info){
      if(!(key==1175)) continue;
      ...
      fit_two_peaks(fo, v_h_event[k], v_h_bg[k], key, val[0], val[1]);
...
void fit_two_peaks(ofstream &f, TH1D *h_event, TH1D *h_bg, int ring, double e1, double e2)
{
  ...
  double x_min = m_2e_info[(int)e][2];
  double x_max = m_2e_info[(int)e][3];
  ...
  par[2] = m_2e_info[e][8]; // gaussian, delta
  par[3] = m_2e_info[e][9]; // Left-Skew, a
  par[4] = m_2e_info[e][10]; // Left-Skew, beta
  par[5] = m_2e_info[e][11]; // Right-Skew, r
  par[6] = m_2e_info[e][12]; // Right-Skew, rho
  par[7] = m_2e_info[e][13]; // Step, s 
  par[8] = m_2e_info[e][1];
  par[9] = h_event->GetBinContent(((int)(e1/bin_width)));
  par[10] = m_2e_info[e][14]; // gaussian, delta
  par[11] = m_2e_info[e][15]; // Left-Skew, a
  par[12] = m_2e_info[e][16]; // Left-Skew, beta
  par[13] = m_2e_info[e][17]; // Right-Skew, r
  par[14] = m_2e_info[e][18]; // Right-Skew, rho
  par[15] = m_2e_info[e][19]; // Step, s 
  par[16] = (h_event->GetBinContent(((int)(x_min/bin_width)))+h_event->GetBinContent(((int)(x_max/bin_width))))/2.; // Background
  par[17] = 0.;
  par[18] = m_2e_info[e][4];
  par[19] = m_2e_info[e][5];
  par[20] = m_2e_info[e][6];
  par[21] = m_2e_info[e][7];
  ...
  TF1 *g2peakexregion_tf = new TF1("g2peakexregion_tf", g2peakexregion_new, x_min, x_max, 22);
  ...
  //
  // peak1
  double energy1 = g2peakexregion_tf->GetParameter(0); 
  double gamma1 = g2peakexregion_tf->GetParameter(1);
  double delta1 = g2peakexregion_tf->GetParameter(2);
  ...
  //
  // peak2
  double energy2 = g2peakexregion_tf->GetParameter(8); 
  double gamma2 = g2peakexregion_tf->GetParameter(9);
  double delta2 = g2peakexregion_tf->GetParameter(2);
  ...
  if(m_e_flag[(int)e]){
    f << e1 << "  " << delta1 << "  " << g2peakexregion_tf->GetParameter(4) << "  " << g2peakexregion_tf->GetParameter(6) << endl;  
    f << e2 << "  " << delta2 << "  " << g2peakexregion_tf->GetParameter(12) << "  " << g2peakexregion_tf->GetParameter(14) << endl;  
...
{
  // read fit par
  ifstream fi_fit_par(Form("./fit_pars/%s_two_peaks.txt",str.c_str())); 
  if(!fi_fit_par){
  ...
  bool flag;
  while(fi_fit_par>>e1>>e2>>x_min>>x_max>>ex_x1>>ex_x2>>ex_x3>>ex_x4>>delta1>>a1>>beta1>>r1>>rho1>>s1>>delta2>>a2>>beta2>>r2>>rho2>>s2>>flag){
    m_2e_info.insert({(int)(e1+e2), {e1,e2,x_min,x_max,ex_x1,ex_x2,ex_x3,ex_x4,delta1,a1,beta1,r1,rho1,s1,delta2,a2,beta2,r2,rho2,s2}});
    m_e_flag.insert({(int)(e1+e2), flag});
  }
  fi_fit_par.close();
...
Double_t g2peakexregion_new(Double_t *v, Double_t *par)
{
  ...
  // peak1
  Double_t gaussian1 = gamma1*exp(-pow((v[0]-energy1)/delta, 2.));
  Double_t left_skew1 = gamma1*1/2.*a1*exp((v[0]-energy1)/beta1);
  left_skew1 *= erfc(delta/2./beta1 + (v[0]-energy1)/delta);
  Double_t right_skew1 = gamma1*1/2.*r1*exp(-(v[0]-energy1)/rho1);
  right_skew1 *= erfc(delta/2./rho1 - (v[0]-energy1)/delta);
  Double_t step1 = gamma1*s1*1/2.*erfc((v[0]-energy1)/delta);

  // peak2
  Double_t gaussian2 = gamma2*exp(-pow((v[0]-energy2)/delta, 2.));
  Double_t left_skew2 = gamma2*1/2.*a2*exp((v[0]-energy2)/beta2);
  left_skew2 *= erfc(delta/2./beta2 + (v[0]-energy2)/delta);
  Double_t right_skew2 = gamma2*1/2.*r2*exp(-(v[0]-energy2)/rho2);
  right_skew2 *= erfc(delta/2./rho2 - (v[0]-energy2)/delta);
  Double_t step2 = gamma2*s2*1/2.*erfc((v[0]-energy2)/delta);
