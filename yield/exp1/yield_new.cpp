...
void fit_peak(ofstream &f, TH1D *h_event, TH1D *h_bg, int ring, double e)
{
  ...
  gpeakexregion_tf->FixParameter(2, par[2]);
  gpeakexregion_tf->FixParameter(9, 0.);
  ...
  gpeakexregion_tf->fixparameter(8, gpeakexregion tf->Eval(gpeakexregion tf->Getparameter(0)- par[2] / sqrt(2)* 3.5));
...
bool init_pars(string str="")
{
  ...
  // read delta
  ifstream fi_fit_delta_par(Form("./%s_all_delta_aa.txt",str.c_str())); 
  if(!fi_fit_delta_par){
    std::cout << "can not open fi_fit_delta_par" << std::endl;
    return 0;
  }

  map<int, double> m_e_delta;
  std::getline(fi_fit_delta_par, line);
  while(fi_fit_delta_par>>e>>delta){
    m_e_delta[(int)e] = delta;
  }
  fi_fit_delta_par.close();

  for(auto &[key,val]:m_r_e_info){
    if(m_e_delta.count((int)key.second)){
      m_r_e_info[key][7] = m_e_delta[(int)key.second];
    }
  }
